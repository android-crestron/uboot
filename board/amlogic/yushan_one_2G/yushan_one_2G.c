...
static void gpio_set_vbus_power(char is_power_on)
{
        ...
        {
               //GPIOD_9 -- USB_PWR_CTL
               amlogic_gpio_direction_output(GPIOX_18, 1);

               udelay(100000);
        }
        else
        {
               amlogic_gpio_direction_output(GPIOX_18, 0);
        }
}
...
static void reset_csr_bt(void)
{
	amlogic_gpio_direction_output(GPIOY_8, 1);
	mdelay(5);
	amlogic_set_value(GPIOY_8,0);
	mdelay(10);
	amlogic_set_value(GPIOY_8,1);
}

static void reset_nxp_amp(void)
{
	amlogic_gpio_direction_output(GPIOAO_12, 1);
	mdelay(5);
	amlogic_set_value(GPIOAO_12,1);
	mdelay(10);
	amlogic_set_value(GPIOAO_12,0);
}

static void reset_microsemi_codec(void)
{
	amlogic_gpio_direction_output(GPIOX_20, 1);
        mdelay(5);
        amlogic_set_value(GPIOX_20,0);
        mdelay(10);
        amlogic_set_value(GPIOX_20,1);
}

static void reset_touch(void)
{
    amlogic_gpio_direction_output(GPIODV_0, 1);
    mdelay(5);
    amlogic_set_value(GPIODV_0,0);
    mdelay(10);
    amlogic_set_value(GPIODV_0,1);
}

int board_init(void)
{
    /* GPIO reset Touch panel chipset */
    reset_touch();
  ...
  serial_init_uart_a(a_uart);
	...
	/*GPIO Reset CSR BT Chipset*/
	reset_csr_bt();
	
	/*GPIO Reset NXP Amp Chipset*/
	reset_nxp_amp();

	/*GPIO Reset Microsemi Codec Chipset*/
	reset_microsemi_codec();
...
#define HAVE_CAM							(1 << 2)
#define HAVE_MIC							(1 << 3)
#define YUSHAN_5INCH_ALL			(LCD_5INCH | HAVE_CAM | HAVE_MIC)
#define YUSHAN_7INCH_ALL			(LCD_7INCH | HAVE_CAM | HAVE_MIC)
#define YUSHAN_10INCH_ALL			(LCD_10INCH | HAVE_CAM | HAVE_MIC)
...
static int select_m8m2_dtd(unsigned int pID1)
{
	...
	int i = 0;
	int lcd_size = 0, lcm_supplier = 0;
    char lcm_type;
	int model = 0;

    get_lcm_supplier( &lcm_supplier );
    if ( lcm_supplier == LCM_TRULY )
    {
        printf("LCM Supplier: Truly\n");
        setenv("lcmsupplier", "truly");
    }
    else if ( lcm_supplier == LCM_EDT )
    {
        printf("LCM Supplier: EDT\n");
        setenv("lcmsupplier", "edt");
    }
    else
    {
        // default set as TRULY
        printf("LCM Supplier: (Default) Truly\n");
        setenv("lcmsupplier", "truly");
    }

	get_lcd_size(&lcd_size);
	printf("LCD Size:");
	if (lcd_size==LCD_5INCH) {
	    printf(" 5 inch\n");
	}
	else if (lcd_size==LCD_7INCH) {
	    printf(" 7 inch\n");
	}
	else if (lcd_size==LCD_10INCH) {
	    printf(" 10 inch\n");
	}
	else {
	    printf(" No defined\n");
	...
	model = lcd_size | HAVE_CAM | HAVE_MIC;
	for (i = 0; i < CONFIG_NR_DRAM_BANKS; i++) {
		ddr_size += gd->bd->bi_dram[i].size;
	}
	if(0x80000000==ddr_size){/*2G ddr*/
//		setenv("aml_dt", "m8m2_n200_2G");
		...
		check_lcm_type(&lcm_type);
		if (model==YUSHAN_5INCH_ALL) {
			if (lcm_type==0)
				setenv("aml_dt", "yushan_one_old5inch");
			else
				setenv("aml_dt", "yushan_one_5inch");
		}
		else if (model==YUSHAN_7INCH_ALL) {
			if (lcm_type==0)
				setenv("aml_dt", "yushan_one_old7inch");
			else
				setenv("aml_dt", "yushan_one_7inch");
		}
		else if (model==YUSHAN_10INCH_ALL) {
			if (lcm_type==0)
				setenv("aml_dt", "yushan_one_old10inch");
			else
				setenv("aml_dt", "yushan_one_10inch");
		}
		else {
			printf("Model Type is No Defined !!!\n");
			return -1;
		}

		run_command("lcd dpck", 0);
