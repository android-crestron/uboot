...
void emmc_env_relocate_spec(void)
{
	...
	u64 cnt = CONFIG_ENV_SIZE;
//	u64 blk = 0;
	u64 blk = CONFIG_ENV_START;
	uint32_t crc;

		dev_num = CONFIG_SYS_SD_ENV_DEV;
//	struct mmc *mmc = NULL;
//	char *name = "env";
...
//		set_default_env("!No MMC device");
//		goto error;
		set_default_env("!No SD CARD device");		
		return;
	}
//	if ((cnt % 512) || (blk % 512)) {
//	    set_default_env("!addr or blk count notalign");
...
//	 cnt  = cnt >> blk_shift;
	if (mmc_init(mmc)) {
		set_default_env("!SD CARD init failed");
		return ;
	}
...
//	ret =(cnt == mmc->block_dev.block_read(dev_num, blk, cnt, env_buf));
	...
	if(!ret){
//		set_default_env("!readenv() failed");
		set_default_env("!readenv() SD CARD failed");
		saveenv();
		goto error;
	}
...
//		set_default_env("!bad CRC");
		set_default_env("!bad CRC IN SD CARD");
		saveenv();
	}
//	
//	env_import((const char *)env_buf, 1);	 
...
//		puts("No MMC device! Environment MMC not initialized!\n");
		puts("No SD CARD device! Environment SD not initialized!\n");
		return -1;
	}
	if ((cnt % 512) || (blk % 512)) {
...
//        printf("mmc save env ok\n");
        printf("sd card save env ok\n");
    }
    else
    {
//        printf("mmc save env fail\n");
//       free(env_new_p);
        printf("sd card save env fail\n");
        free(env_new_p);
		return -1;
    }	
