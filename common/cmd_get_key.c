            ...
            if(!strncmp(secukey_cmd, "read", strlen("read"))) {
                ...
                if(error >= 0) {    //read success
                    ...
                    for(i=0,j=0; i<KEY_BYTES*2; i++,j++) {
                    	  k = i+1;
                        buf[j]= (((asc_to_hex(databuf[i]))<<4) | (asc_to_hex(databuf[k])));
                        i++;
                    }
                    printf("%s is: ", namebuf);
...
static int do_get_mac(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if(!strncmp(device, "flash", strlen("flash"))) {
        ...
        ret = read_secukey(Argc, (const char **)sArgv, buff);
        if(!ret) {                                      
    ...
    /* read from efuse */
        ...
        ret = read_efuse(Argc, (const char **)eArgv, buff);
        if(!ret) {
            for(i=0; i<6; i++) {
                if(buff[i] != 0x00) {
                    simbol = 1;
