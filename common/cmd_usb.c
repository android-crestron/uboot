...
int do_usb(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	block_dev_desc_t *stor_dev;
#endif
	char filecontent[150] = {0}, filename[3][50] = {{0},{0},{0}};
	char *target;
	int size, image_idx = 0;
	...
	if (strncmp(argv[1], "factory", 7) == 0) {
		size = file_fat_read("jabil.txt", (unsigned char *)filecontent, 0);
		if(size==-1) {
			printf("Couldn't open jabil.txt file");
			return 1;
		}
		target = strtok(filecontent, "\n");
		strcpy(filename[0], target);
		target = strtok(NULL, "\n");
		strcpy(filename[1], target);
		target = strtok(NULL, "\n");
		strcpy(filename[2], target);
		
		if ((target = getenv ("lcdsize")) != NULL) {
			if (strncmp(target, "5inch", 5) == 0) {
				image_idx = 0;
			} else if (strncmp(target, "7inch", 5) == 0) {
				image_idx = 1;
			} else if (strncmp(target, "10inch", 6) == 0) {
				image_idx = 2;
			}
			printf("factory image file name : %s\n", filename[image_idx]);
			setenv("factoryimage", filename[image_idx]);
		}
