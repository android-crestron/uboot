...
#include <post.h>
#include "crestronUbootVersion.h"
...
//	char *r_efus;
	...
	if (readl(P_AO_RTI_STATUS_REG2) == 0x1)
	{
		setenv ("softreboot", "1");
	}
	else
	...
	checkBootRetry();
	setenv("crestron_uboot_version",  crestron_uboot_version);
	printf ("Final crestron_uboot_version: %s\n", crestron_uboot_version);
	saveenv(); // Added
